#/////////////////////////////////////////////////
__author__      = "Chengshun Shang (Utrecht University)"
__copyright__   = "Copyright (C) 2026-present by Chengshun Shang"
__version__     = "0.0.1"
__maintainer__  = "Chengshun Shang"
__email__       = "c.shang@uu.nl"
__status__      = "development"
__date__        = "June 28, 2026"
__license__     = "MIT License"
#/////////////////////////////////////////////////

from pathlib import Path

from fastslippy import FastSlipPy
from fastslippy.pre_processing.model_parameters import ModelParameters


YR = 365 * 24 * 3600.0
OUTPUT_DIR = (
    Path("output")
    / "bp3_90_real_dx100_ycore60_newton_tol5_surface_extrap"
)

class RunFastSlipPy(FastSlipPy):
    """
    This can be customized for specific runs.
    """
    def run(self):
        super().run()

        self.grid.plot_grid()
        self.grid.plot_mesh()


def build_bp3_90_parameters() -> ModelParameters:
    """Return the reproducible 90-degree BP3-QD comparison case.

    The complete 40 km rate-and-state fault is kept inside the 100 m uniform
    down-dip core.  Coarsening the mesh inside that interval can turn one
    system-wide rupture into alternating full and partial events, which makes
    both recurrence periods and station peak rates appear unstable.
    """
    params = ModelParameters(
        case_type = "california",
        alpha = 90.0,
        motion_sign = -1,
        auto_motion_sign = True,
        xsize = 80e3,
        ysize = 80e3,
        # 100 m in the x core and throughout the complete frictional fault.
        # The remaining far field is quadratically stretched.
        Nx = 361,
        Ny = 681,
        Nt = 100000,
        output_interval = 10,
        checkpoint_interval = 1000,
        rho = 2670.0,
        cs = 3464,
        mu0 = 0.6,
        nu = 0.25,
        #E=0.55e10, #according to k_critical = sigam * (b-a) / d_c, E = 1e10
        V0 = 1e-6,
        a0 = 0.01,
        a_max = 0.025,
        b0 = 0.015,
        L = 0.008,
        dt_init = 1.0,
        dt_max = 0.1 * YR,
        dt_growth = 1.2,
        tfinal = 500 * YR,
        friction_tolerance = 5.0,
        output_vtk_option = False,
        Vi = 1e-9,
        flash_heating_option = False,
        extrapolate_surface_fault_rate = True,
        H = 15e3,
        h = 3e3,
        W_f = 40e3,
        x_stretch_enabled=True,
        y_stretch_enabled=True,
        x_stretch_inner_size=20e3,
        y_stretch_inner_size=60e3,
        x_stretch_inner_points=201,
        y_stretch_inner_points=601,
        x_stretch_power=2,
        y_stretch_power=2,
        x_stretch_max_cell_size=650.0,
        y_stretch_max_cell_size=400.0,
        allow_nonuniform_solver=True,
    )

    params.loading.tload = 0.0
    params.loading.dPdt_pre = 0.0
    params.loading.dPdt_post = 0.0
    params.loading.V_p = 1e-9
    params.loading.V_L = 1e-9

    params.bc.left.ux.set_fixed()
    params.bc.left.uy.set_velocity(-0.5 * params.loading.V_p)
    params.bc.right.ux.set_fixed()
    params.bc.right.uy.set_velocity(0.5 * params.loading.V_p)
    params.bc.top.set_traction_free()
    params.bc.bottom.set_traction_free()

    top_of_layer = params.ysize
    bottom_of_layer = params.ysize * 2
    params.layers.set_homogeneous(top = top_of_layer, bottom = bottom_of_layer, a=params.a0, b=params.b0)

    return params


if __name__ == "__main__":
    params = build_bp3_90_parameters()

    # Keep this run isolated from older dataall/checkpoint files.  Reusing a
    # generic output directory makes an interrupted run easy to compare against
    # a stale dataall.npz from a different grid.
    model = RunFastSlipPy(params=params, output_dir=str(OUTPUT_DIR))
    model.run()
