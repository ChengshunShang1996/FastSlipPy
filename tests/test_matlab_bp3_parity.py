...
def test_stretched_vertical_bp3_three_steps_remain_close_to_matlab_reference(tmp_path):
    """Regression-test the production Python loop against a MATLAB checkpoint.

    The compact reference case retains the BP3 material and friction laws while
    reducing the domain enough for the normal test suite.  It deliberately uses
    alpha=90, stretching in both directions, side loading, bottom loading, and
    the production safeguarded-Newton friction solve.  The reference checkpoint
    was generated by ``perfect_code_matlab_version.m`` with MATLAB R2026a; its
    exact run configuration is recorded beside the MAT file.

    MATLAB uses bisection and Python uses safeguarded Newton.  In addition, the
    Python stretched-grid operator now evaluates mixed derivatives and surface
    tractions at their actual staggered coordinates; the saved MATLAB operator
    used compact differences scaled by a local mean spacing.  The physical
    state must remain close, but off-fault fields are no longer coefficient-for-
    coefficient identical to that legacy discretisation.
    """
    ...
    params = _bp3_parameters(
        ...
        x_stretch_enabled=True,
        y_stretch_enabled=True,
        x_stretch_inner_size=800.0,
        y_stretch_inner_size=400.0,
        x_stretch_inner_points=9,
        y_stretch_inner_points=5,
        x_stretch_power=2,
        y_stretch_power=2,
        allow_nonuniform_solver=True,
        motion_sign=-1,
    ...
    params.bc.bottom.uy.set_velocity(0.5e-9)

    model = FastSlipPy(params=params, output_dir=str(tmp_path))
    model.figure_creator.plot_results = lambda *args, **kwargs: None
    model.run()

    assert model.grid.cosa == 0.0
    assert model.grid.is_nonuniform
    np.testing.assert_array_equal(
        model.grid.x,
        [-1600, -1150, -800, -550, -400, -300, -200, -100, 0,
         100, 200, 300, 400, 550, 800, 1150, 1600],
    )
    np.testing.assert_array_equal(
        model.grid.y,
        [0, 100, 200, 300, 400, 525, 700, 925, 1200],
    )
    np.testing.assert_array_equal(model.output.tm, [1.0, 2.0, 3.0])

    reference_path = (
        Path(__file__).parent
        / "reference_data"
        / "matlab_bp3_90_step3.mat"
    )
    reference = loadmat(reference_path, squeeze_me=True)
    actual = {
        "U": model.fault.U,
        "V": model.fault.V,
        "tau": model.fault.tau,
        "sigma": model.fault.sigma,
        "theta": model.fault.theta,
        "tauqs": model.tauqs,
        "sigmaqs": model.sigmaqs,
        "uy": model.uy,
        "vy": model.vy,
        "ux": model.ux,
        "vx": model.vx,
    ...
    for field, values in actual.items():
        ...
        np.testing.assert_allclose(
            ...
            rtol=rtol,
            atol=atol,
            err_msg=f"90-degree MATLAB parity failed for {field}",
        )

    assert float(reference["t"]) == 3.0
    assert float(reference["dt"]) == 1.0
